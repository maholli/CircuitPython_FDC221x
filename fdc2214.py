...
class FDC2214(object):
    ...
    def channel(self, channel=2):
        if channel not in (0,1,2,3,10,23):
        ...
        if channel == 10:
            self._write16(FDC2214_MUX_CONFIG, 0x820D)
            self._write16(FDC2214_CONFIG,     0x1E01)
        if channel == 23:
            self._write16(FDC2214_MUX_CONFIG, 0xC20D)
            self._write16(FDC2214_CONFIG,     0x1E01)
    ...
    def scan(self,channels=10):
        ...
        _cha = _chb = 0
        if channels == 10: 
            _cha = (self._read16(FDC2214_DATA_CH0_MSB) & FDC2214_DATA_CHx_MASK_DATA) << 16
            _cha |= self._read16(FDC2214_DATA_CH0_LSB)
            _chb = (self._read16(FDC2214_DATA_CH1_MSB) & FDC2214_DATA_CHx_MASK_DATA) << 16
            _chb |= self._read16(FDC2214_DATA_CH1_LSB)
        elif channels == 23:
            _cha = (self._read16(FDC2214_DATA_CH2_MSB) & FDC2214_DATA_CHx_MASK_DATA) << 16
            _cha |= self._read16(FDC2214_DATA_CH2_LSB)
            _chb = (self._read16(FDC2214_DATA_CH3_MSB) & FDC2214_DATA_CHx_MASK_DATA) << 16
            _chb |= self._read16(FDC2214_DATA_CH3_LSB)
        try:
